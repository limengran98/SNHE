...
def load_acm(ratio, type_num):
    ...
    pap = th.FloatTensor(normalize_adj(pap).todense())
    psp = th.FloatTensor(normalize_adj(psp).todense())
    I = sp.eye(feat_p.shape[1])
    I = th.FloatTensor(normalize_adj(I).todense())
    pos = sparse_mx_to_torch_sparse_tensor(pos)
    train = [th.LongTensor(i) for i in train]
    val = [th.LongTensor(i) for i in val]
    test = [th.LongTensor(i) for i in test]
    ae_path = '../data/acm/ACM.pkl'
    return [nei_a, nei_s], [feat_p, feat_a, feat_s], [pap, psp], pos, label, train, val, test, ae_path
...
def load_dblp(ratio, type_num):
    ...
    feat_p = th.FloatTensor(preprocess_features(feat_p))
    feat_a = th.FloatTensor(preprocess_features(feat_a))
    apa = th.FloatTensor(normalize_adj(apa).todense())
    apcpa = th.FloatTensor(normalize_adj(apcpa).todense())
    aptpa = th.FloatTensor(normalize_adj(aptpa).todense())
    I = sp.eye(feat_p.shape[1])
    I = th.FloatTensor(normalize_adj(I).todense())
    pos = sparse_mx_to_torch_sparse_tensor(pos)
    train = [th.LongTensor(i) for i in train]
    val = [th.LongTensor(i) for i in val]
    test = [th.LongTensor(i) for i in test]
    ae_path = '../data/dblp/DBLP.pkl'
    return [nei_p], [feat_a, feat_p], [apa, apcpa, aptpa], pos, label, train, val, test, ae_path
...
def load_imdb(ratio, type_num):
    ...
    feat_p = th.FloatTensor(preprocess_features(feat_p))
    mam = th.FloatTensor(normalize_adj(mam).todense())
    mdm = th.FloatTensor(normalize_adj(mdm).todense())
    I = sp.eye(feat_p.shape[1])
    I = th.FloatTensor(normalize_adj(I).todense())
    pos = sparse_mx_to_torch_sparse_tensor(pos)
    train = [th.LongTensor(i) for i in train]
    val = [th.LongTensor(i) for i in val]
    test = [th.LongTensor(i) for i in test]
    ae_path = '../data/imdb/IMDB.pkl'
    return [nei_p], [feat_m, feat_p], [mam, mdm], pos, label, train, val, test, ae_path
...
def load_aminer(ratio, type_num):
    ...
    pap = th.FloatTensor(normalize_adj(pap).todense())
    prp = th.FloatTensor(normalize_adj(prp).todense())
    pos = sparse_mx_to_torch_sparse_tensor(pos)
    train = [th.LongTensor(i) for i in train]
    val = [th.LongTensor(i) for i in val]
    test = [th.LongTensor(i) for i in test]
    ae_path = '../data/aminer/aminer.pkl'
    return [nei_a, nei_r], [feat_p, feat_a, feat_r], [pap, prp], pos, label, train, val, test, ae_path
...
def load_freebase(ratio, type_num):
    ...
    mam = th.FloatTensor(normalize_adj(mam).todense())
    mdm = th.FloatTensor(normalize_adj(mdm).todense())
    mwm = th.FloatTensor(normalize_adj(mwm).todense())
    pos = sparse_mx_to_torch_sparse_tensor(pos)
    train = [th.LongTensor(i) for i in train]
    val = [th.LongTensor(i) for i in val]
    test = [th.LongTensor(i) for i in test]
    ae_path = '../data/freebase/freebase.pkl'
    return [nei_d, nei_a, nei_w], [feat_m, feat_d, feat_a, feat_w], [mdm, mam, mwm], pos, label, train, val, test, ae_path
